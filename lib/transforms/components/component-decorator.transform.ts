...
const inlineCSSFromFile = (
  ...
  if (oleUtil.hasKey(objLitExp, STYLE_URLS_PROPERTY_NAME)) {
    ...
    const styleUrlsArrayLE = oleUtil.getPropertyAsArrayLiteralExpression(
      properties,
      STYLE_URLS_PROPERTY_NAME
    );
    if (styleUrlsArrayLE) {
...
export function inlineCSSFromFileTransformer<T extends ts.Node>(
  filepath: string
