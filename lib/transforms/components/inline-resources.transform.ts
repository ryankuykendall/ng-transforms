...
import { Property as cdProperty } from './../../declaration-metadata/component-decorator.properties';
...
// OLD METHOD...delete this after refactor...
const inlineComponentAssetContentsFromModelTransform = (model: IComponentInlineModel) => {
  ...
  const updatedOLE: ts.ObjectLiteralExpression = ts.createObjectLiteral(updatedOLEProperties);
  const callExpression = ts.getMutableClone(model.callExpression);
  callExpression.expression = updatedOLE;
  model.callExpression = callExpression;
  // model.callExpression.expression = updatedOLE;
};
